...
app.get('/dashboard/initiative/:id', async (req, res) => {
  ...
  res.type('html').send(`<!doctype html><html><head>${uiHead('Initiative Detail')}</head><body><div class="app-shell">
    ...
    <div class="card mt-3"><div class="card-body"><div class="d-flex justify-content-between align-items-center mb-2"><strong>Recent Events</strong><span class="small text-muted">Showing ${eventSlice.length} of ${events.length}</span></div><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>TIME</th><th>ACTOR</th><th>EVENT</th></tr></thead><tbody>${eventRows}</tbody></table></div><div class="d-flex justify-content-between align-items-center mt-2"><div class="small text-muted">Page ${eventPageClamped} / ${eventTotalPages}</div><div class="btn-group btn-group-sm"><a class="btn btn-outline-secondary ${eventPageClamped<=1?'disabled':''}" href="/dashboard?eventPage=${Math.max(1,eventPageClamped-1)}&eventPageSize=${eventPageSize}">Prev</a><a class="btn btn-outline-secondary ${eventPageClamped>=eventTotalPages?'disabled':''}" href="/dashboard?eventPage=${Math.min(eventTotalPages,eventPageClamped+1)}&eventPageSize=${eventPageSize}">Next</a><a class="btn btn-outline-secondary" href="/dashboard/events">Full log</a></div></div></div></div>
  </div>
  <script>
