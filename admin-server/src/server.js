...
app.get('/dashboard/initiative/:id', async (req, res) => {
  ...
  res.type('html').send(`<!doctype html><html><head>${uiHead('Initiative Detail')}</head><body><div class="app-shell">
    ...
    <div class="card mt-3"><div class="card-body"><div class="d-flex justify-content-between align-items-center"><strong>Recent Events</strong><span class="small text-muted">${events.length} total events</span></div><div class="small text-muted mt-2">Event rows moved to dedicated page to keep dashboard compact.</div><div class="mt-2"><a class="btn btn-sm btn-outline-secondary" href="/dashboard/events">Open event log</a></div></div></div>
  </div>
  <script>
