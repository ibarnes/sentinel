...
const DASHBOARD_MEETING_MINUTES_SCHEMA_FILE = path.join(ROOT, 'dashboard', 'meeting-minutes.schema.v1.json');
...
function cleanRoleLabel(v) {
  const s = String(v || '');
  return s.replace(/\s*\(Primary[^)]*\)?/gi, '').replace(/\s+/g, ' ').trim();
}
...
function normalizeMeetingMinutesRecord(row = {}) {
  const toList = (v) => Array.isArray(v)
    ? v.map((x) => String(x || '').trim()).filter(Boolean)
    : [];

  const nextActions = Array.isArray(row.next_actions)
    ? row.next_actions.map((a) => ({
        action: String(a?.action || '').trim(),
        owner: String(a?.owner || '').trim(),
        due: String(a?.due || '').trim(),
        status: String(a?.status || 'open').trim() || 'open',
      })).filter((a) => Boolean(a.action || a.owner || a.due))
    : [];

  const sourceObj = row.source && typeof row.source === 'object' && !Array.isArray(row.source)
    ? row.source
    : {};
  ...
  const normalized = {
    ...
    participants: toList(row.participants),
    summary: String(row.summary || '').trim(),
    initiative_ids: toList(row.initiative_ids),
    buyer_ids: toList(row.buyer_ids),
    signal_ids: toList(row.signal_ids),
    decisions: toList(row.decisions),
    risks: toList(row.risks),
    next_actions: nextActions,
    source: {
      type: String(sourceObj.type || '').trim(),
      link: String(sourceObj.link || sourceObj.url || '').trim(),
      recorded_by: String(sourceObj.recorded_by || '').trim(),
      submitted_by: String(sourceObj.submitted_by || '').trim(),
      team_member: String(sourceObj.team_member || '').trim(),
      confidence: String(sourceObj.confidence || '').trim(),
      notes: String(sourceObj.notes || '').trim(),
    },
    visibility: String(row.visibility || 'internal').trim() || 'internal',
...
async function readMeetingMinutes() {
  const rows = await readJson(DASHBOARD_MEETING_MINUTES_FILE, []);
  return (Array.isArray(rows) ? rows : []).map((r) => normalizeMeetingMinutesRecord(r));
}
...
app.get('/dashboard/actors', requireAnyAuth, async (req, res) => {
  ...
  const sort = String(req.query.sort || 'last').trim();
  ...
  const sortKeyMap = {
    first: (a) => String(a.first || ''),
    last: (a) => String(a.last || ''),
    designation: (a) => String(a.designation || ''),
    company: (a) => String(a.company || ''),
    country: (a) => String(a.country || '')
  };
  const keyFn = sortKeyMap[sort] || sortKeyMap.last;
  ...
  const filtered = normalized.filter((a) => {
    ...
    const cmp = av.localeCompare(bv) || `${a.last} ${a.first}`.trim().localeCompare(`${b.last} ${b.first}`.trim());
...
app.post('/api/actors', requireRole('architect','editor'), async (req, res) => {
  const actor_id = String(req.body.actor_id || '').trim();
  const name = String(req.body.name || '').trim();
  const actor_type = String(req.body.actor_type || 'other').trim();
  if (!actor_id || !name) return res.status(400).send('actor_id and name required');
  const actors = await readJson(DASHBOARD_ACTORS_FILE, []);
  if (actors.some((a)=>String(a.actor_id||'')===actor_id)) return res.status(409).send('actor already exists');
  actors.push({ actor_id, name, actor_type, created_at: nowIso() });
  await writeJson(DASHBOARD_ACTORS_FILE, actors);
...
app.post('/api/signals-feed/refresh', requireRole('architect','editor'), async (_req, res) => {
  ...
  try {
    ...
    const scriptPath = path.join(ROOT, 'scripts/signals-feed/build-signals-feed.mjs');
    const run = spawnSync('node', [scriptPath], { cwd: ROOT, encoding: 'utf8', timeout: 180000 });
    if (run.error || run.status !== 0) {
      const detail = run.error ? String(run.error.message || run.error) : String(run.stderr || run.stdout || 'unknown error');
      await appendAuditEvent({
        ts: nowIso(),
        actor: getUserLabel(_req),
        role: effectiveRole(_req) || 'unknown',
        event_type: 'workflow.error',
        entity_type: 'signals_feed',
        entity_id: 'refresh',
...
app.get('/dashboard/signal/:id', async (req, res) => {
  ...
  const buyers = await readJson(path.join(ROOT, 'dashboard/data/buyers.json'), []);
  const byId = Object.fromEntries(buyers.map((b) => [b.buyer_id, b.name]));
  const s = signals.find((x) => String(x.signal_id || '') === id);
  if (!s) return res.status(404).type('html').send(`<!doctype html><html><head>${uiHead('Signal Not Found')}</head><body><div class="app-shell">${dashboardNav('signals')}<a class="btn btn-sm btn-outline-secondary mb-2" href="/dashboard/signals">← Signals</a><div class="alert alert-warning">Signal not found.</div></div></body></html>`);
  const statusBadge = (() => {
    const v = String(s.status || 'Monitor');
    const cls = v === 'Verified' ? 'success' : (v === 'Actioned' ? 'primary' : 'secondary');
    return `<span class="badge text-bg-${cls}">${escapeHtml(v)}</span>`;
  })();
...
app.get('/dashboard/actions', async (req, res) => {
  ...
  const meetingMinutes = await readMeetingMinutes();
  ...
  const blockerRank = (r) => {
    if (r.escalationActive) return 5;
    if (r.overdue) return 4;
    if (r.blocked) return 3;
    if (r.due72) return 2;
    if ((r.waiting_on || []).length || String(r.blocker_reason || '').trim()) return 1;
    return 0;
  };
  ...
  const filtered = [...filteredUnsorted].sort((a, b) => {
    ...
    let cmp = 0;
    if (sortBy === 'action') cmp = compareText(a.action_text, b.action_text);
    else if (sortBy === 'owner') cmp = compareText(a.owner, b.owner);
    else if (sortBy === 'decision_owner') cmp = compareText(a.decision_owner, b.decision_owner);
    else if (sortBy === 'due') cmp = compareText(a.due || '9999-12-31', b.due || '9999-12-31');
    else if (sortBy === 'status') cmp = compareText(a.status, b.status);
    else if (sortBy === 'priority') cmp = compareText(a.priority, b.priority);
  ...
  const makeActionsUrl = ({ targetMode = isAdvanced ? 'advanced' : 'simple', quickOverride, sortByOverride, sortDirOverride } = {}) => {
    const params = new URLSearchParams();
    ...
    const effectiveSortBy = sortByOverride === undefined ? sortBy : sortByOverride;
    const effectiveSortDir = sortDirOverride === undefined ? sortDir : sortDirOverride;
    if (effectiveSortBy) {
      params.set('sort_by', effectiveSortBy);
      params.set('sort_dir', effectiveSortDir === 'desc' ? 'desc' : 'asc');
    }
  ...
  const sortMark = (key) => sortBy === key ? (sortDir === 'desc' ? ' ↓' : ' ↑') : '';
  const sortHref = (key, targetMode = isAdvanced ? 'advanced' : 'simple') => {
    const nextDir = sortBy === key && sortDir === 'asc' ? 'desc' : 'asc';
    return makeActionsUrl({ targetMode, sortByOverride: key, sortDirOverride: nextDir });
  };
  ...
  const simpleFilterForm = `
    <form method="get" action="/dashboard/actions" class="row g-2 mb-3">
      ...
      ${sortBy ? `<input type="hidden" name="sort_by" value="${escapeHtml(sortBy)}"/><input type="hidden" name="sort_dir" value="${escapeHtml(sortDir)}"/>` : ''}
...
app.get('/dashboard/meetings', async (req, res) => {
  const meetings = await readMeetingMinutes();
  ...
  const cards = sorted.map((m) => {
    ...
    const recordedBy = String(m?.source?.recorded_by || m?.source?.team_member || '').trim();
    const submittedBy = String(m?.source?.submitted_by || '').trim();
    return `<a class="card mb-3 text-decoration-none" style="color:inherit" href="/dashboard/meeting/${encodeURIComponent(String(m.meeting_id || ''))}"><div class="card-body"><div class="d-flex justify-content-between align-items-start gap-2"><div><h6 class="mb-1" style="color:var(--text-primary);font-size:1.2rem">${escapeHtml(m.title || m.meeting_id || 'Untitled meeting')}</h6><div class="small text-muted mono">${escapeHtml(String(m.meeting_id || ''))}</div></div><div class="small mono" style="color:var(--text-muted)">${escapeHtml(dt || '—')}</div></div><div class="small mt-2" style="color:var(--text-primary)">${escapeHtml(m.summary || '—')}</div><div class="small mt-2" style="color:var(--text-primary)"><strong>Initiatives:</strong> ${escapeHtml(initiatives)}</div><div class="small mt-1" style="color:var(--text-primary)"><strong>Buyers:</strong> ${escapeHtml(buyers)}</div><div class="small mt-1" style="color:var(--text-primary)"><strong>Signals:</strong> ${escapeHtml(signals)}</div><div class="small mt-1" style="color:var(--text-primary)"><strong>Action items:</strong> ${escapeHtml(String(actions))}</div>${recordedBy ? `<div class="small mt-1 text-muted"><strong>Recorded by:</strong> ${escapeHtml(recordedBy)}</div>` : ''}${submittedBy ? `<div class="small mt-1 text-muted"><strong>Submitted by:</strong> ${escapeHtml(submittedBy)}</div>` : ''}</div></a>`;
  }).join('');

  res.type('html').send(`<!doctype html><html><head>${uiHead('Meeting Minutes')}</head><body><div class="app-shell">
    ${dashboardNav('meetings')}
    ${pageHeader('Meeting Minutes', '<a class="btn btn-sm btn-outline-secondary" href="/dashboard/meetings/export.json">Get JSON</a><a class="btn btn-sm btn-outline-secondary" href="/dashboard/meetings/schema.json">Schema</a>', 'Meeting records linked to initiatives, buyers, signals, and team source metadata')}
...
app.get('/dashboard/meetings/schema.json', async (_req, res) => {
  const schema = await readJson(DASHBOARD_MEETING_MINUTES_SCHEMA_FILE, meetingMinutesSchemaV1());
  res.type('application/json').send(JSON.stringify(schema, null, 2));
});
...
app.get('/dashboard/contracts/export.csv', requireAnyAuth, async (_req, res) => {
  const p = path.join(ROOT, 'contracts.csv');
...
app.get('/dashboard/initiative/:id', async (req, res) => {
  ...
  let weakestLayer = LAYERS[0];
  for (const l of LAYERS) if (perLayer[l] < perLayer[weakestLayer]) weakestLayer = l;
  const statusUrgency = { missing: 300, identified: 220, validated: 140, engaged: 60, committed: 0 };
  const gateStage = String(i.gate_stage || 'Gate 1');
  const gateRelevanceBoost = (r) => {
    const layer = String(r.layer || '');
    const roleId = String(r.role_id || '');
    const role = String(r.role_label || '').toLowerCase();
    if (gateStage === 'Gate 0' || gateStage === 'Gate 1') {
      if (layer === 'political') return 95;
      if (layer === 'development') return 85;
      if (role.includes('regulator') || roleId.includes('regulator')) return 95;
      return 30;
    }
    if (gateStage === 'Gate 2') {
      if (layer === 'capital') return 95;
      if (layer === 'asset') return 80;
      if (role.includes('demand') || roleId.includes('off_taker')) return 90;
      return 35;
    }
    if (gateStage === 'Gate 3') {
      if (layer === 'capital') return 90;
      if (layer === 'delivery') return 80;
      if (role.includes('epc') || role.includes('supplier') || roleId.includes('lender')) return 90;
      return 40;
    }
    return layer === 'delivery' ? 80 : 45;
  };
  const priorityScore = (r) => {
    const constraintBoost = primaryConstraint && String(r.role_id || '') === String(primaryConstraint.role_id || '') ? 1000 : 0;
    const criticalBoost = r?.is_critical_role ? 220 : 0;
    const statusWeight = statusUrgency[String(r?.status || 'missing')] ?? 0;
    const weakestLayerBoost = String(r.layer || '') === String(weakestLayer) ? 120 : 0;
    const gateBoost = gateRelevanceBoost(r);
    return constraintBoost + criticalBoost + statusWeight + weakestLayerBoost + gateBoost;
  };
  const priorityTier = (r) => {
    const status = String(r?.status || 'missing');
    const isConstraint = primaryConstraint && String(r.role_id || '') === String(primaryConstraint.role_id || '');
    if (isConstraint) return 'P0';
    if (Boolean(r?.is_critical_role) && ['missing','identified','validated'].includes(status)) return 'P1';
    if (Boolean(r?.is_critical_role) && ['engaged','committed'].includes(status)) return 'P2';
    if (!Boolean(r?.is_critical_role) && ['missing','identified','validated','engaged'].includes(status)) return 'P3';
    return 'P4';
  };
  const priorityBadgeClass = (tier) => ({ P0:'text-bg-danger', P1:'text-bg-warning', P2:'text-bg-info', P3:'text-bg-primary', P4:'text-bg-secondary' }[tier] || 'text-bg-secondary');
  const priorityMeaning = (tier) => ({
    P0: 'Primary constraint',
    P1: 'Critical blocker',
    P2: 'Important structural role',
    P3: 'Supporting role',
    P4: 'Non-critical / informational'
  }[tier] || 'Non-critical / informational');
  const rolesPrioritized = rolesAll
    .map((r) => ({ ...r, priority_score: priorityScore(r), priority_tier: priorityTier(r) }))
    .sort((a, b) => Number(b.priority_score || 0) - Number(a.priority_score || 0));
  const macroGravityRendered = renderSimpleMarkdown(String(i.macro_gravity_summary || ''));
  ...
  const initiativeTransitions = [...(transitionsAll || [])]
    .filter((t) => Array.isArray(t.initiative_ids) && t.initiative_ids.includes(i.initiative_id))
    .sort((a, b) => String(b.timestamp || b.created_at || '').localeCompare(String(a.timestamp || a.created_at || '')));
  const lastTransition = initiativeTransitions[0] || null;
  const currentStateFromTransition = String(lastTransition?.to_state || '').trim();
  const currentState = currentStateFromTransition || String(i.current_state || 'constraint_discovery');
  const nextRequiredState = String(i.next_required_state || stateDefs?.states?.[currentState]?.next_state || '—');
  const readiness = Number(i.readiness_score ?? 0);
  const maxPossible = Number(i.max_possible_score ?? stateDefs?.states?.[currentState]?.max_score_ceiling ?? 100);
  const readinessExplain = i.readiness_explain || {};
  const lastDelta = readinessExplain.last_delta;
  const capReason = String(readinessExplain.cap_reason || '—');
  const constraintsOpen = (constraintsAll || []).filter((c) => String(c.initiative_id || '') === i.initiative_id && String(c.status || 'open') !== 'resolved');
  const sevBase = { critical: 100, high: 70, medium: 40, low: 15 };
  const scoredConstraints = constraintsOpen.map((c) => {
    let score = Number(sevBase[String(c.severity || 'medium').toLowerCase()] || 20);
    if (Array.isArray(c.blocking_states) && c.blocking_states.map(String).includes(nextRequiredState)) score += 25;
    if (String(c.priority || '') === 'P0') score += 20;
    if (Boolean(c.is_critical_path)) score += 15;
    if (String(c.status || '') === 'in_progress') score -= 20;
    return { ...c, _score: score };
  }).sort((a,b)=>Number(b._score||0)-Number(a._score||0));
  const statePrimaryConstraintObj = scoredConstraints[0] || null;
  const statePrimaryConstraint = String(i.primary_constraint || statePrimaryConstraintObj?.name || '—');
  const nextRequiredTransition = String(i.next_required_transition || statePrimaryConstraintObj?.linked_transition_required || '—');
  const confidence = lastTransition?.confidence == null ? '—' : String(lastTransition.confidence);
  const lastTransitionType = String(lastTransition?.transition_type || '—');
  const lastTransitionTs = String(lastTransition?.timestamp || lastTransition?.created_at || '').slice(0,16).replace('T',' ');
  const criticalPathYes = Boolean(statePrimaryConstraintObj?.is_critical_path || nextRequiredTransition !== '—');
  const regressionFlag = String(lastTransitionType).toLowerCase() === 'regression';
  ...
  res.type('html').send(`<!doctype html><html><head>${uiHead('Initiative Detail')}</head><body><div class="app-shell">
    ...
    <details class="card mb-3"><summary class="card-header"><strong>Actor Alignment Map</strong> <span class="small text-muted ms-2">(collapsible)</span></summary><div class="card-body">
      ...
      <div class="row g-2 mb-3">
        ...
        <div class="col-6 col-md-4 col-lg-2"><div class="border rounded p-2"><div class="small text-muted">Weakest Layer</div><div class="text-capitalize"><strong>${escapeHtml(weakestLayer)}</strong></div></div></div>
        <div class="col-6 col-md-4 col-lg-3"><div class="border rounded p-2"><div class="small text-muted">Primary Constraint</div><div><strong>${escapeHtml(primaryConstraint ? (cleanRoleLabel(primaryConstraint.role_label) || primaryConstraint.role_id || 'None') : 'None')}</strong></div></div></div>
      ...
      <details class="mb-3"><summary><strong>Map Views</strong> <span class="small text-muted">(collapsed by default)</span></summary>
        <div class="pt-2">
          <div class="d-flex gap-2 mb-2">
          ...
          <div id="actor-view-layer" class="row g-2 mb-3">
            ${LAYERS.map((l)=>`<div class="col-12 col-lg"><div class="border rounded p-2 h-100"><div class="d-flex justify-content-between"><strong>${escapeHtml(LAYER_LABEL[l])}</strong><span class="small text-muted">${perLayer[l]}%</span></div><div class="mt-2 d-flex flex-column gap-1">${(actorLayers[l]||[]).map((r)=>`<div class="border rounded px-2 py-1 small ${r?.is_critical_role ? 'border-danger-subtle' : ''}">${escapeHtml(STATUS_ICON[String(r?.status||'missing')] || '⬜')} ${escapeHtml(cleanRoleLabel(r.role_label) || r.role_id || 'Unnamed role')}${r?.mapped_entity_ref ? ` <span class="text-muted">— ${escapeHtml(String(r.mapped_entity_ref))}</span>` : ''}${r?.is_critical_role ? ' <span class="badge text-bg-light border ms-1">Critical</span>' : ''}</div>`).join('') || '<div class="small text-muted">No roles defined</div>'}</div></div></div>`).join('')}
          </div>

          <div id="actor-view-status" class="mb-3" style="display:none">
            ${['missing','identified','validated','engaged','committed'].map((s)=>`<div class="mb-2"><div><strong>${escapeHtml(s)}</strong></div><div class="d-flex flex-wrap gap-1 mt-1">${rolesAll.filter((r)=>String(r?.status||'missing')===s).map((r)=>`<span class="badge text-bg-light border">${escapeHtml(LAYER_LABEL[r.layer])}: ${escapeHtml(cleanRoleLabel(r.role_label) || r.role_id || 'Unnamed role')}${r?.is_critical_role ? ' *' : ''}</span>`).join('') || '<span class="small text-muted">None</span>'}</div></div>`).join('')}
          </div>
        </div>
      </details>

      <details class="mb-2"><summary><strong>Primary Constraint Panel</strong></summary><div class="card mt-2"><div class="card-body py-2">
        <div><strong>Primary Constraint:</strong> ${escapeHtml(primaryConstraint ? (cleanRoleLabel(primaryConstraint.role_label) || primaryConstraint.role_id || 'None') : 'None')}</div>
      ...
      <details><summary><strong>Actor Role Detail Table</strong></summary><div class="table-responsive mt-2"><table class="table table-sm align-middle mb-0"><thead><tr><th>#</th><th>Priority</th><th>Layer</th><th>Role</th><th>Status</th><th>Mapped Entity</th><th>Critical</th><th>Owner</th><th>Due</th>${canEdit && editMode ? '<th></th>' : ''}</tr></thead><tbody>
        ${rolesPrioritized.map((r, idx)=>`<tr data-priority-score="${escapeHtml(String(Math.round(Number(r.priority_score||0))))}"><td class="text-muted">${idx + 1}</td><td><span class="badge ${priorityBadgeClass(r.priority_tier)} priority-badge" title="${escapeHtml(priorityMeaning(r.priority_tier || 'P4'))}" data-priority-meaning="${escapeHtml(priorityMeaning(r.priority_tier || 'P4'))}" tabindex="0" role="button" aria-label="${escapeHtml((r.priority_tier || 'P4') + ': ' + priorityMeaning(r.priority_tier || 'P4'))}">${escapeHtml(r.priority_tier || 'P4')}</span></td><td>${escapeHtml(LAYER_LABEL[r.layer])}</td><td>${escapeHtml(cleanRoleLabel(r.role_label) || r.role_id || 'Unnamed role')} <span class="badge text-bg-light border role-def-badge" data-role-definition="${escapeHtml(String(r?.notes || '').trim() || String(r?.role_definition || '').trim() || 'No role notes yet. Use Edit to add notes.')}" title="${escapeHtml(String(r?.notes || '').trim() || String(r?.role_definition || '').trim() || 'No role notes yet. Use Edit to add notes.')}" tabindex="0" role="button" aria-label="${escapeHtml(`Role details: ${String(r?.notes || '').trim() || String(r?.role_definition || '').trim() || 'No role notes yet. Use Edit to add notes.'}`)}">?</span></td><td>${escapeHtml(String(r.status || 'missing'))}</td><td>${escapeHtml(String(r.mapped_entity_ref || '—'))}</td><td>${r?.is_critical_role ? 'Yes' : 'No'}</td><td>${escapeHtml(String(r.owner || '—'))}</td><td>${escapeHtml(String(r.due || '—'))}</td>${canEdit && editMode ? `<td class="text-end"><details><summary class="btn btn-sm btn-outline-secondary">Edit</summary><div class="card p-2 mt-1" style="min-width:360px"><form method="post" action="/api/initiatives/${encodeURIComponent(i.initiative_id)}/actor-roles/update" class="row g-1"><input type="hidden" name="role_id" value="${escapeHtml(String(r.role_id || ''))}"/><div class="col-12"><label class="form-label small mb-0">Role Label</label><input class="form-control form-control-sm" name="role_label" value="${escapeHtml(String(r.role_label || ''))}" /></div><div class="col-12"><label class="form-label small mb-0">Role Definition</label><input class="form-control form-control-sm" name="role_definition" value="${escapeHtml(String(r.role_definition || ''))}" /></div><div class="col-6"><label class="form-label small mb-0">Status</label><select class="form-select form-select-sm" name="status">${['missing','identified','validated','engaged','committed'].map((s)=>`<option value="${s}" ${String(r.status||'missing')===s?'selected':''}>${s}</option>`).join('')}</select></div><div class="col-6"><label class="form-label small mb-0">Entity Type</label><select class="form-select form-select-sm" name="mapped_entity_type"><option value="" ${!r.mapped_entity_type?'selected':''}>(none)</option>${['buyer','organization','team','regulator','operator','other'].map((t)=>`<option value="${t}" ${String(r.mapped_entity_type||'')===t?'selected':''}>${t}</option>`).join('')}</select></div><div class="col-12"><label class="form-label small mb-0">Linked Entity</label><input class="form-control form-control-sm" name="mapped_entity_ref" list="entityPickerList" value="${escapeHtml(String(r.mapped_entity_ref || ''))}"/></div><div class="col-6"><label class="form-label small mb-0">Owner</label><input class="form-control form-control-sm" name="owner" value="${escapeHtml(String(r.owner || ''))}"/></div><div class="col-6"><label class="form-label small mb-0">Due</label><input class="form-control form-control-sm" type="date" name="due" value="${escapeHtml(String(r.due || ''))}"/></div><div class="col-12"><label class="form-label small mb-0">Notes</label><input class="form-control form-control-sm" name="notes" value="${escapeHtml(String(r.notes || ''))}"/></div><div class="col-6 d-flex align-items-end"><div class="form-check"><input class="form-check-input" type="checkbox" name="is_critical_role" id="crit_${escapeHtml(String(r.role_id||''))}" ${r?.is_critical_role ? 'checked' : ''}/><label class="form-check-label" for="crit_${escapeHtml(String(r.role_id||''))}">Critical</label></div></div><div class="col-6 d-flex justify-content-end gap-1 align-items-end"><button class="btn btn-sm btn-primary" type="submit">Save</button></form><form method="post" action="/api/initiatives/${encodeURIComponent(i.initiative_id)}/actor-roles/${encodeURIComponent(String(r.role_id || ''))}/delete" onsubmit="return confirm('Remove role ${escapeHtml(r.role_label || r.role_id || '')}?');"><button class="btn btn-sm btn-outline-danger" type="submit">Remove</button></form></div></form></div></details></td>` : ''}</tr>`).join('') || `<tr><td colspan="${canEdit && editMode ? '10' : '9'}" class="text-muted">No actor roles defined. Apply Template or + Add Actor Role to begin.</td></tr>`}
    ...
    <div class="card mt-3"><div class="card-body"><div class="d-flex justify-content-between align-items-center mb-2"><strong>Recent Events</strong></div><div class="small text-muted">Event stream temporarily unavailable on this page. Use <a href="/dashboard/events">Full log</a>.</div></div></div>
  </div>
  <script>
    (function setupPriorityTooltips(){
      ...
      const wire = (selector, attr) => {
        document.querySelectorAll(selector).forEach((el) => {
          if (attr) {
            const v = el.getAttribute(attr);
            if (v) el.setAttribute('data-priority-meaning', v);
          }
          el.addEventListener('mouseenter', () => showTip(el));
          el.addEventListener('mouseleave', hideTip);
          el.addEventListener('focus', () => showTip(el));
          el.addEventListener('blur', hideTip);
          el.addEventListener('click', (e) => { e.preventDefault(); showTip(el); setTimeout(hideTip, 2000); });
          el.addEventListener('touchstart', () => { showTip(el); setTimeout(hideTip, 2000); }, { passive: true });
        });
      };
      wire('.priority-badge');
      wire('.role-def-badge', 'data-role-definition');
...
function meetingMinutesSchemaV1() {
  return {
    $schema: 'http://json-schema.org/draft-07/schema#',
    title: 'USG Meeting Minutes Schema v1',
    type: 'array',
    items: {
      type: 'object',
      required: ['meeting_id', 'title', 'participants', 'summary'],
      properties: {
...
async function ensureTeamAndBoardFiles() {
  ...
  if (!fssync.existsSync(DASHBOARD_MEETING_MINUTES_SCHEMA_FILE)) {
    await fs.writeFile(DASHBOARD_MEETING_MINUTES_SCHEMA_FILE, JSON.stringify(meetingMinutesSchemaV1(), null, 2));
  }
...
function evaluateBoardStateGate({ task, targetStatus, initiativesById, transitionsByInitiative, definitions, gateChecks }) {
  ...
  for (const iid of taskInitiatives) {
    ...
    const mandateGateStates = new Set(definitions?.mandate_gate?.required_for_states || ['governance_ready','capital_ready','execution_ready']);
    if (mandateGateStates.has(requiredState)) {
      if (!initiative.mandate_signed || !initiative.fee_secured) {
        return { ok: false, reason: `${iid}: mandate gate blocked (mandate_signed + fee_secured required)` };
      }
    }
...
async function loadActorEntityOptions() {
  const buyers = await readJson(path.join(ROOT, 'dashboard/data/buyers.json'), []);
  const actors = await readJson(DASHBOARD_ACTORS_FILE, []);
  const opts = [];
  for (const b of buyers) {
    opts.push({ ref: String(b.buyer_id || ''), label: String(b.name || b.buyer_id || ''), type: 'buyer' });
  }
  for (const a of actors) {
    opts.push({ ref: String(a.actor_id || ''), label: String(a.name || a.actor_id || ''), type: String(a.actor_type || 'actor') });
  }
  return opts.filter((x) => x.ref);
}

async function isKnownEntityRef(value) {
...
app.post('/api/initiatives/:id/actor-roles/add', requireRole('architect','editor'), async (req, res) => {
  ...
  const mappedRef = String(req.body.mapped_entity_ref || '').trim();
  if (mappedRef && !(await isKnownEntityRef(mappedRef))) return res.status(400).send('mapped_entity_ref must be selected from Actors/Buyers registry');
...
app.post('/api/initiatives/:id/actor-roles/update', requireRole('architect','editor'), async (req, res) => {
  ...
  row.role_definition = String(req.body.role_definition || '').trim();
  ...
  row.mapped_entity_ref = mappedRef || null;
