...
app.get('/dashboard/initiative/:id', async (req, res) => {
  ...
  res.type('html').send(`<!doctype html><html><head>${uiHead('Initiative Detail')}</head><body><div class="app-shell">
    ...
    <div class="card mb-3"><div class="card-body">
      ...
      <div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Layer</th><th>Role</th><th>Status</th><th>Mapped Entity</th><th>Critical</th><th>Owner</th><th>Due</th><th>Notes</th>${canEdit && editMode ? '<th></th>' : ''}</tr></thead><tbody>
        ${rolesAll.map((r)=>`<tr><td>${escapeHtml(LAYER_LABEL[r.layer])}</td><td>${escapeHtml(r.role_label || r.role_id || 'Unnamed role')}</td><td>${escapeHtml(String(r.status || 'missing'))}</td><td>${escapeHtml(String(r.mapped_entity_ref || '—'))}</td><td>${r?.is_critical_role ? 'Yes' : 'No'}</td><td>${escapeHtml(String(r.owner || '—'))}</td><td>${escapeHtml(String(r.due || '—'))}</td><td>${escapeHtml(String(r.notes || ''))}</td>${canEdit && editMode ? `<td class="text-end"><form method="post" action="/api/initiatives/${encodeURIComponent(i.initiative_id)}/actor-roles/${encodeURIComponent(String(r.role_id || ''))}/delete" onsubmit="return confirm('Remove role ${escapeHtml(r.role_label || r.role_id || '')}?');"><button class="btn btn-sm btn-outline-danger" type="submit">Remove</button></form></td>` : ''}</tr>`).join('') || `<tr><td colspan="${canEdit && editMode ? '9' : '8'}" class="text-muted">No actor roles defined. Apply Template or + Add Actor Role to begin.</td></tr>`}
      </tbody></table></div>
    </div></div>
