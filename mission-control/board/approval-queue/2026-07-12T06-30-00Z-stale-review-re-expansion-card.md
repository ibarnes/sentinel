...
## Why the bucket widened again
- The July 11 recovery sweep correctly contracted the stale-review count to `11` because `TASK-0029`, `TASK-0030`, and `TASK-0269` were still under `24h` old from the July 10 linkage refresh at that execution timestamp.
- At `2026-07-12T06:30:00Z`, `TASK-0029` and `TASK-0030` are now about `47.97h` old from that same refresh, so both rows truthfully re-enter the stale `Ready for Review >24h` bucket.
- `TASK-0269` does not join the stale-review bucket because it remains tracked as a blocked parent rather than as an unattended `Ready for Review` row.
- The truthful July 12 surface is therefore `14` stale review rows plus blocked parents `TASK-0107` / `TASK-0269`, not the prior `11`-row shape.
