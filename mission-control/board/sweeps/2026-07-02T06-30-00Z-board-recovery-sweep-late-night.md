...
- `TASK-0029`, `TASK-0030`, `TASK-0107`, and `TASK-0269` are each exactly `24.0h` old at this sweep boundary, so they remain open but do not enter the strict stale-RFR count yet.
...
- Why this won:
  ...
  - the real live nuance since the July 1 sweep is that the compact lane stayed exact-threshold stable after yesterday's refresh rather than drifting into a hidden stale-parent expansion
  - one bounded routing artifact could lock that no-drift proof and preserve the smallest valid owner decision surface without violating any blocked apply boundary

### Subtask executed
- `TASK-0516` - Publish exact-threshold no-drift stalled-lane card
- Timebox: `30-45 min`
- Acceptance criteria:
  ...
  - Artifact records the exact `24.0h` no-drift threshold condition for `TASK-0029`, `TASK-0030`, `TASK-0107`, and `TASK-0269` without mutating any board status.
  - Board linkage points the live parent rows to the July 2 recovery surfaces.
...
## Board linkage
- updated `mission-control/board/BOARD.json` to attach the July 2 recovery surfaces to `TASK-0107`, `TASK-0029`, `TASK-0030`, and `TASK-0269`
- added completed subtask `TASK-0516`
