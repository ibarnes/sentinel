...
- Stalled open set at actual sweep execution time (`2026-07-17T06:30:00Z`):
  - `0` `Doing >48h`
...
- Executed one new bounded recovery child under `TASK-0107` so the blocked lane reflects a fresh board-native reread instead of stale carry-forward narrative:
  - `TASK-0581` - Publish July 17 unchanged stale-review decision surface card
...
### Truthful delta versus July 16
- No queue-shape delta: the stalled surface remains `14` stale review rows plus blocked parents `TASK-0107` / `TASK-0269`.
- No rows changed status, no new freshening touches landed, and no new honest autonomous execution path appeared inside the stalled lane since `2026-07-16T06:30:00Z`.
...
- Acceptance criteria:
  ...
  - Artifact confirms the July 17 stalled surface is unchanged versus July 16 because no row changed status or freshness state inside the stalled lane.
  ...
  - Board linkage points the live blocked parent rows to the July 17 recovery surfaces.

## Artifacts produced
- `mission-control/board/approval-queue/2026-07-17T06-30-00Z-stale-review-unchanged-decision-surface-card.md`
- `mission-control/board/sweeps/2026-07-17T06-30-00Z-board-recovery-sweep-late-night.md`

## Board linkage
- updated `mission-control/board/BOARD.json` to attach the July 17 recovery surfaces to `TASK-0107` and `TASK-0269`
- added completed subtask `TASK-0581`
