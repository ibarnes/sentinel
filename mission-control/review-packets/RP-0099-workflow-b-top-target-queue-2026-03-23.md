# RP-0099 — Workflow B Top Target Queue — 2026-03-23

## Observations
- Top-10 buyer hygiene remains incomplete: all top-10 entries still miss core metadata fields (`hq`, `region`, `buyer_role`, `buyer_class`).
...
- High-influence/no-path gaps remain active for top-ranked buyers:
  ...
  - DFC: Ben Black
  - DFC: Conor Coleman
- Warming/blocked recency SLA breach (>14d) remains active in top-10 set:
  - TAFF `PATH-TAFF-FAISAL-NET-001` (Warming, ~17.6d)
- Additional recency pressure remains on key ready lanes (>21d):
  - PIF `PATH-PIF-001` (~24.5d, Ready)
  - AFC `PATH-AFC-001` (~25.5d, Ready)
...
2. **ADQ** — Priority 10 (Urgency 3 / Fit 2 / Staleness 5)
   - Why now: same structural gap profile (no DA, no path, no metadata).
   - Recommended next touch: map sponsor/operator pair and assign path owner with first-touch date.
3. **MUBADALA** — Priority 10 (Urgency 3 / Fit 2 / Staleness 5)
...
4. **WORLDBANK** — Priority 10 (Urgency 3 / Fit 2 / Staleness 5)
...
7. **PIF** — Priority 10 (Urgency 3 / Fit 2 / Staleness 5)
   - Why now: no DA coverage and path recency drift remains >24 days.
   - Recommended next touch: refresh path with dated re-engagement and add DA entries linked to current initiatives.
8. **AFC** — Priority 10 (Urgency 3 / Fit 2 / Staleness 5)
   - Why now: no DA coverage and path recency drift remains >25 days.
...
10. **USVI-RECOVERY-AUTHORITY** — Priority 10 (Urgency 4 / Fit 2 / Staleness 4)
   - Why now: high-influence federal oversight node is not mapped to a direct access path.
   - Recommended next touch: add a direct path entry for HUD/FEMA oversight bodies with owner, channel, and next-touch date.
...
- Apply a **14-day recency SLA** for all top-10 warming/blocked paths with mandatory blocker note if exceeded.

## Next Actions (for Isaac approval)
1. Approve same-day metadata normalization for all top-10 buyers.
2. Approve DA baseline creation for PIF, TEMASEK, ADQ, AFC, MUBADALA, WORLDBANK, QIA.
3. Approve named-path remediation for DFC, TAFF, and USVI-RECOVERY-AUTHORITY high-influence stakeholders.
