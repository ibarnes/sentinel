# Workflow B Top Target Queue — 2026-05-01
...
- Signal-pressure delta: new_high_impact_count=0 (generated_at 2026-05-01T07:11:06.538Z).
...
- Missing decision architecture and path drift are treated as conversion blockers for outreach planning.
...
1. **Alpha Wave Global** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   ...
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
2. **Brookfield Infrastructure Partners L.P.** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   - Why now: missing decision architecture; metadata drift; no mapped access path.
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
3. **General Atlantic** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   - Why now: missing decision architecture; metadata drift; no mapped access path.
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
4. **Global Infrastructure Partners** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   - Why now: missing decision architecture; metadata drift; no mapped access path.
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
5. **Haun Ventures** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   - Why now: missing decision architecture; metadata drift; no mapped access path.
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
6. **KKR & Co. Inc.** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   - Why now: missing decision architecture; metadata drift; no mapped access path.
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
...
8. **Tariq Al Futtaim Family Foundation (TAFF)** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   ...
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
9. **USVI Recovery Program Authority (ODR + VIHFA)** — Priority 11 (Urgency 4 / Buyer Fit 2 / Staleness 5)
   - Why now: metadata drift; stale blocked/warming path >14d.
   - Recommended next touch: Create/refresh DA baseline (economic + technical owner) and open/refresh dated access path.
10. **Public Investment Fund** — Priority 9 (Urgency 4 / Buyer Fit 2 / Staleness 3)
...
3. Approve daily enforcement escalation for repeat pathless/stale buyers.
