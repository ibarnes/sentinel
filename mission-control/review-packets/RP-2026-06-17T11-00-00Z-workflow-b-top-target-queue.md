# Workflow B Top Target Queue - 2026-06-17

## Observations
- Top-10 buyer ranking is unchanged from the prior cycle: USVI Recovery Program Authority (ODR + VIHFA), Alpha Wave Global, Global Infrastructure Partners, Stonepeak, Haun Ventures, Brookfield Infrastructure Partners L.P., KKR & Co. Inc., Tariq Al Futtaim Family Foundation (TAFF), General Atlantic, Public Investment Fund.
- Decision-architecture coverage is still missing for 8/10 top buyers; only USVI Recovery Program Authority and TAFF currently carry any mapped decision roles.
- Contact-path coverage is still missing for 7/10 top buyers; USVI Recovery Program Authority (ODR + VIHFA), Tariq Al Futtaim Family Foundation (TAFF), Public Investment Fund are the only ranked accounts with any path coverage, and 2/10 ranked buyers still carry warming paths older than 14 days.
- Ranked-record metadata quality is still degraded for 10/10 buyers because every top-10 record is missing at least `region`, `buyer_role`, or `buyer_class`; Public Investment Fund is still the weakest record because it also lacks `hq_country`.
- Signal-pressure delta remained fresh from `2026-06-17T08:11:18.051Z` with `new_high_impact_count=0` and `new_signal_count=0`, so the current queue is still driven by structural access-graph gaps rather than a new event spike.

## Assumptions
- This run uses local snapshots only: `dashboard/data/buyers.json`, `dashboard/data/decision_architecture.json`, `dashboard/data/contact_paths.json`, and the current fresh `mission-control/signal-pressure/out/pressure-delta.json`.
...
1. **USVI Recovery Program Authority (ODR + VIHFA)** - Score 2.7
   - Why now: it is still the highest-ranked buyer and the strongest access-graph record in the queue, but the federal oversight lane remains stuck in `Warming` roughly 91 days after its last update.
...
8. **Tariq Al Futtaim Family Foundation (TAFF)** - Score 2.3
   - Why now: TAFF already has direct access and decision-role coverage, but one strategic network path is still stuck in `Warming` roughly 103 days after its last update.
