# Workflow B Top Target Queue - 2026-06-18
...
- Signal-pressure delta remained fresh from `2026-06-18T08:41:40.029Z` with `new_high_impact_count=0` and `new_signal_count=0`, so the current queue is still driven by structural access-graph gaps rather than a new event spike.
...
1. **USVI Recovery Program Authority (ODR + VIHFA)** - Score 2.7
   - Why now: it is still the highest-ranked buyer and the strongest access-graph record in the queue, but the federal oversight lane remains stuck in `Warming` roughly 92 days after its last update.
...
8. **Tariq Al Futtaim Family Foundation (TAFF)** - Score 2.3
   - Why now: TAFF already has direct access and decision-role coverage, but one strategic network path is still stuck in `Warming` roughly 104 days after its last update.
