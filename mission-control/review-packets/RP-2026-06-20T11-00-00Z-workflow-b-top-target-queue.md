# Workflow B Top Target Queue - 2026-06-20

## Observations
- Top-10 buyer ranking for this run resolves from `dashboard/data/buyers.json` to: USVI Recovery Program Authority (ODR + VIHFA), Alpha Wave Global, Global Infrastructure Partners, Stonepeak, Haun Ventures, Brookfield Infrastructure Partners L.P., KKR & Co. Inc., Tariq Al Futtaim Family Foundation (TAFF), General Atlantic, and Public Investment Fund.
...
- Two ranked buyers already carry stale warming lanes older than 14 days: USVI Recovery Program Authority still has `PATH-USVI-FED-001` in `Warming` status last touched `2026-03-17` (about 94 days old), and TAFF still has `PATH-TAFF-FAISAL-NET-001` in `Warming` status last touched `2026-03-05` (about 106 days old).
...
- Signal-pressure delta remained fresh from `2026-06-20T06:11:30.318Z` with `new_high_impact_count=0` and `new_signal_count=0`, so the queue is still driven by access-readiness and graph-quality gaps rather than a new event spike.
...
1. **USVI Recovery Program Authority (ODR + VIHFA)** - Score 2.7
   - Why now: it remains the highest-ranked buyer and the strongest access-graph record in the queue, but the federal oversight lane is still stuck in `Warming` roughly 94 days after its last update.
...
8. **Tariq Al Futtaim Family Foundation (TAFF)** - Score 2.3
   - Why now: TAFF already has direct access and decision-role coverage, but one strategic network path is still stuck in `Warming` roughly 106 days after its last update.
