# Workflow B Top Target Queue - 2026-06-23
...
- One ranked buyer already has a stale warming lane: Nigeria Sovereign Investment Authority still carries `PATH-NSIA-001` in `Warming` status, last touched `2026-02-25`, which is now roughly 118 days old.
...
- Signal-pressure delta was refreshed successfully at `2026-06-23T11:01:12.490Z` with `new_high_impact_count=0` and `new_signal_count=0`, so the queue remains driven by graph-quality and access-readiness gaps rather than a new event spike.
...
1. **Public Investment Fund** - Score 2.2
   - Why now: it remains the first-ranked buyer and one of only three current top-10 buyers with any mapped path, but there is still no decision-architecture coverage for the actual investment-committee route.
