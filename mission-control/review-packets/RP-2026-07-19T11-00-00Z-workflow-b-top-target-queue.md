# Workflow B Top Target Queue - 2026-07-19
...
- The ranked set is unchanged versus `2026-07-18`; no new entrant displaced the current sovereign and DFI-heavy top 10 queue.
...
- Contact-path coverage is still missing for 7/10 ranked buyers; only Public Investment Fund, Africa Finance Corporation, Nigeria Sovereign Investment Authority currently carry any path record in `dashboard/data/contact_paths.json`.
- 1 ranked buyer still carries a stale warming lane older than 14 days: Nigeria Sovereign Investment Authority still has `PATH-NSIA-001` in `Warming` status with `last_touch_at=2026-02-25` and `next_touch_at=2026-03-05`, so the path is now about 144 days stale and still unresolved.
...
- Signal-pressure freshness was checked before this run; current delta remains `generated_at=2026-07-19T07:41:21.990Z` with `new_high_impact_count=0` and `new_signal_count=0`, so the queue is still driven by access-readiness and graph coverage rather than a new event spike.
- Workflow A materially changed the Nigeria reading before this packet: `FMF-NG` recovered from the `https://www.finance.gov.ng/category/press-release/` `HTTP_404` path to `fetchStatus=OK` in the 10:30 UTC run, restoring `date`, `capitalAmount`, `mandateLanguage`, and `leadershipStatement`; the remaining gap there is now path creation, not evidence recovery.
...
1. **Public Investment Fund** - Score 2.2
   - Why now: it still sits first in the canonical queue, there is still no mapped decision architecture.
...
2. **Gulf Investment Corporation** - Score 1.8
   - Why now: there is still no mapped decision architecture, path coverage is still zero.
...
4. **Nigeria Sovereign Investment Authority** - Score 1.9
   - Why now: there is still no mapped decision architecture, its only active route is now stale in `Warming` status.
...
5. **Federal Ministry of Finance (Nigeria)** - Score 1.8
   - Why now: there is still no mapped decision architecture, path coverage is still zero, Workflow A just recovered the evidence row, so the remaining drag is now access creation rather than source degradation.
...
- Treat FMF-NG as the clearest near-term conversion window because the evidence row recovered in Workflow A while Workflow B still shows zero path coverage.
