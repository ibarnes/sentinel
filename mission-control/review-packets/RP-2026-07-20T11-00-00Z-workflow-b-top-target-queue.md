# Workflow B Top Target Queue - 2026-07-20
...
- The ranked set is unchanged versus `2026-07-19`; no new entrant displaced the current sovereign and DFI-heavy top 10 queue.
...
- 1 ranked buyer still carries a stale warming lane older than 14 days: Nigeria Sovereign Investment Authority still has `PATH-NSIA-001` in `Warming` status with `last_touch_at=2026-02-25` and `next_touch_at=2026-03-05`, so the path is now about 145 days stale and still unresolved.
...
- Signal-pressure freshness was checked before this run; current delta remains `generated_at=2026-07-20T08:41:20.054Z` with `new_high_impact_count=0` and `new_signal_count=0`, so the queue is still driven by access-readiness and graph coverage rather than a new event spike.
- Workflow A held flat on the buyer evidence surface before this packet: the 10:30 UTC run preserved `FMF-NG` as a recovered `fetchStatus=OK` row with no buyer-field deltas versus the Sunday, July 19, 2026 archived run, so the remaining gap there is still path creation rather than evidence recovery.
