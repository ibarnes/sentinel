# Workflow B Top Target Queue - 2026-07-23
...
- The ranked set is unchanged versus Wednesday, July 22, 2026; no new entrant displaced the current sovereign and DFI-heavy top 10 queue.
...
- 1 ranked buyer still carries a stale warming lane older than 14 days: Nigeria Sovereign Investment Authority still has `PATH-NSIA-001` in `Warming` status with `last_touch_at=2026-02-25` and `next_touch_at=2026-03-05`, so the path is now about 148 days stale and still unresolved.
...
- Signal-pressure freshness was checked before this run; current delta is still the Thursday, July 23, 2026 07:41:30.326 UTC snapshot in `mission-control/signal-pressure/out/pressure-delta.json` with `new_high_impact_count=0` and `new_signal_count=0`, so the queue is still driven by access-readiness and graph coverage rather than a new event spike.
- Workflow A improved the buyer evidence surface before this packet: the 10:30 UTC run restored `FMF-NG` from Wednesday, July 22, 2026's broken `fetchStatus=HTTP_404` row to `fetchStatus=OK`, with `date`, `capitalAmount`, `mandateLanguage`, and `leadershipStatement` all repopulated, so the FMF-NG lane is back to a pure access-creation gap rather than a combined evidence-and-access failure.
...
5. **Federal Ministry of Finance (Nigeria)** - Score 1.8
   - Why now: there is still no mapped decision architecture and path coverage is still zero, but Workflow A has restored the evidence row, so the main remaining gap is opening the first fiscal approval route against a now-trustworthy mandate surface.
...
- Treat FMF-NG as a recovered evidence surface that still needs first-pass access creation while the buyer row remains fresh.
...
3. Approve first-pass path creation for FMF-NG now that the buyer evidence surface has recovered.
