...
function bulletsToHtml(items) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (text.startsWith('__INTRO__ ')) {
      out.push(`<li class="intro">${sanitizeHtml(text.replace('__INTRO__ ', ''))}</li>`);
      parentOpen = false;
      continue;
    }

    if (i === 0 && /:\s*$/.test(text) && arr.length > 1) {
      out.push(`<li class="intro">${sanitizeHtml(text)}</li>`);
      parentOpen = false;
      continue;
    }

    if (isParent(text)) {
      out.push(`<li class="parent">${sanitizeHtml(text)}</li>`);
      parentOpen = true;
      continue;
    }
    ...
    if (parentOpen) {
      if (isSiblingParent(text)) {
        ...
        out.push(`<li class="sub">${sanitizeHtml(text)}</li>`);
      }
    } else if (isParent(prev)) {
      out.push(`<li class="sub">${sanitizeHtml(text)}</li>`);
      parentOpen = true;
    } else {
      out.push(`<li>${sanitizeHtml(text)}</li>`);
    }
  }
...
function parsePromptSlides(prompt) {
  ...
  for (let i = 0; i < m.length; i++) {
    ...
    let title = `Slide ${m[i][1]}`;
    let rest = body;
    if (bIdx > 0) {
      title = body.slice(0, bIdx).replace(/^title\s*:\s*/i, '').replace(/^[-•\d.\s)]+/, '').replace(/[.:-]+$/, '').trim();
      rest = body.slice(bIdx).trim();
    }
    let bullets = /[•\n]/.test(rest)
      ? rest.split(/[•\n]+/).map((x) => x.trim()).filter(Boolean)
      : rest.split(/[.?!]+/).map((x) => x.trim()).filter(Boolean);
...
function titleFromPurpose(purpose) {
  if (/inevitability/i.test(purpose)) return 'Gravity';
  if (/mandate/i.test(purpose)) return 'Mandate Mirror';
  if (/activation/i.test(purpose)) return 'Controlled Activation';
  if (/decision|close/i.test(purpose)) return 'Decision Gate';
  return 'Structural Progression';
}

function deriveVisualSpec(slide, idx) {
  if (slide.layout !== 'diagram') return null;
  const nodes = ['Pressure Surface', 'Mandate Fit', 'Execution Gate', 'Capital Release'];
  return {
    diagram_type: 'flow-map',
    caption: 'Sequenced controls reduce execution risk while preserving speed.',
    nodes,
    edges: [
      ['Pressure Surface', 'Mandate Fit'],
      ['Mandate Fit', 'Execution Gate'],
      ['Execution Gate', 'Capital Release']
    ],
    palette_hint: idx % 2 === 0 ? 'cool-blue' : 'neutral-slate'
  };
}

function chooseLayout(index, total) {
  if (index === 0) return 'section';
  if (index === Math.floor(total / 2)) return 'diagram';
  if (index === total - 1) return 'proof';
  return index % 3 === 0 ? 'proof' : 'section';
}

function planSlides({ initiative, buyer, prompt }) {
  ...
  const storyline = {
    narrative_arc: ['Context', 'Constraint', 'Control', 'Decision'],
    continuity_anchor: simplify(initiative.name || initiative.initiative_id || 'initiative')
  };
  ...
  if (parsed.length >= 3) {
    ...
    return {
      storyline,
      slides: parsed.map((p, idx) => ({
        purpose: idx < baseline.length ? baseline[idx].purpose : 'Prove structural progression',
        layout: chooseLayout(idx, total),
        title: p.title || titleFromPurpose(idx < baseline.length ? baseline[idx].purpose : ''),
        bullets: p.bullets,
        beat: storyline.narrative_arc[Math.min(storyline.narrative_arc.length - 1, Math.floor((idx / Math.max(1, total - 1)) * storyline.narrative_arc.length))]
      }))
    };
  }

  return { storyline, slides: baseline.map((x, i) => ({ ...x, beat: storyline.narrative_arc[Math.min(i + 1, storyline.narrative_arc.length - 1)] })) };
}
  ...
  if (img.provider === 'openai' && openai) {
    try {
      const r = await fetch('https://api.openai.com/v1/images/generations', {
        ...
        body: JSON.stringify({ model: 'gpt-image-1', prompt: img.prompt, size: '1024x1024' })
      });
      if (r.ok) {
        const j = await r.json();
        const b = j.data?.[0]?.b64_json;
        if (b) {
          await fs.writeFile(out, Buffer.from(b, 'base64'));
          return 'generated';
        }
      }
    } catch {}
...
async function renderSlide(deckDir, templateId, slide) {
  ...
  const kicker = slide.copy.kicker || slide.purpose || 'STRUCTURAL PROOF';
  const html = layout
    .replaceAll('{{title}}', sanitizeHtml(slide.copy.title || slide.title))
    .replaceAll('{{kicker}}', sanitizeHtml(kicker))
    .replaceAll('{{subtitle}}', sanitizeHtml(slide.copy.subtitle || ''))
    .replaceAll('{{footer}}', sanitizeHtml(slide.copy.footer || ''))
    .replaceAll('{{bullets}}', bullets)
    .replaceAll('{{image}}', slide.images?.[0]?.output_path ? `../${path.basename(slide.images[0].output_path)}` : '');

  const css = await fs.readFile(path.join(tRoot, 'slide.css'), 'utf8');
  const continuity = slide.continuity?.to_next ? `<p class="continuity-note">Next: ${sanitizeHtml(slide.continuity.to_next)}</p>` : '';
  const visualCaption = slide.visualSpec?.caption ? `<p class="visual-caption">${sanitizeHtml(slide.visualSpec.caption)}</p>` : '';
  const full = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="../slide.css"><style>.bullets{max-width:70ch}.bullets li{margin:.35rem 0}.bullets li.intro{list-style:none;margin-left:0;padding-left:0;font-weight:500;opacity:.95}.bullets li.parent{list-style:none;margin-left:0;padding-left:0;font-weight:600;opacity:.95}.bullets li.sub{margin-left:1.4rem;list-style-type:circle;opacity:.95}.continuity-note,.visual-caption{margin-top:.75rem;font-size:.85rem;opacity:.78;max-width:68ch}</style></head><body>${html}${visualCaption}${continuity}</body></html>`;
...
async function runPipeline(opts) {
  const deckRel = opts.deck_path || (opts.deck_type === 'utc-internal'
    ? `presentations/${opts.initiative_id}/decks/utc-internal`
    : `presentations/${opts.initiative_id}/decks/buyer-alignment/${opts.buyer_id || 'UNKNOWN'}`);
  ...
  const deck = fssync.existsSync(path.join(deckDir, 'deck.json'))
    ? JSON.parse(await fs.readFile(path.join(deckDir, 'deck.json'), 'utf8'))
    : {
      deckPlan: {
        constraints: { framework: ['Gravity', 'Mandate Mirror', 'Controlled Activation'], target_grade_level: '8-9', max_bullets: 6, max_words_per_bullet: 12 },
        audience: 'internal',
        template_id: opts.template_id,
        slide_count: 0,
        deck_type: opts.deck_type,
        initiative_id: opts.initiative_id,
        buyer_id: opts.buyer_id || null
      },
      slides: []
    };
  ...
  const planOutput = planSlides({ initiative, buyer, prompt: opts.prompt });
  const plan = planOutput.slides;
  deck.deckPlan = {
    ...deck.deckPlan,
    template_id: opts.template_id,
    slide_count: plan.length + 2,
    deck_type: opts.deck_type,
    initiative_id: opts.initiative_id,
    buyer_id: opts.buyer_id || null,
    prompt: opts.prompt || '',
    storyline: planOutput.storyline,
    visual_grammar: {
      whitespace_rule: 'max_6_bullets',
      hierarchy_rule: 'single_primary_claim_per_slide',
      contrast_rule: 'template-first with restrained accent'
    }
  };
  ...
  slides.push({
    ...
    copy: {
      ...
      subtitle: simplify(opts.prompt || 'Structural proof deck'),
      footer: `${opts.initiative_id} • ${opts.deck_type}`
    },
    title: initiative.name,
    bullets: [],
    images: [],
    continuity: { to_next: plan[0]?.title || 'Structural narrative begins' },
    constraints: { max_bullets: 6, max_words_per_bullet: 12, target_grade_level: '8-9' },
    provenance: { sources: [] },
    qa: {},
    revisions_count: 0
  });
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    const continuity = {
      from_prev: i === 0 ? initiative.name : plan[i - 1].title,
      to_next: i + 1 < plan.length ? plan[i + 1].title : 'Decision Gate',
      anchor: planOutput.storyline.continuity_anchor
    };
    let slide = {
      slide_id: id,
      purpose: p.purpose,
      layout: p.layout,
      copy: {
        kicker: p.beat || 'Progression',
        title: p.title,
        bullets: p.bullets,
        subtitle: i > 0 ? `Builds on: ${continuity.from_prev}` : '',
        footer: `${opts.initiative_id} • ${opts.deck_type}`
      },
      title: p.title,
      bullets: p.bullets,
      visualSpec: deriveVisualSpec(p, i),
      images: [{ slot_id: 'img-001', prompt: `Minimal infographic for ${p.title}. No text, strong hierarchy, clear negative space.`, provider: opts.image_provider, output_path: `${deckRel}/assets/${id}-img-001.png` }],
      continuity,
      constraints: { max_bullets: 6, max_words_per_bullet: 12, target_grade_level: '8-9' },
      provenance: { sources: [] },
      qa: {},
    ...
    await log('draft', 'ok', { slide_id: id, layout: p.layout, beat: p.beat });

    for (let r = 0; r < 3; r++) {
      const issues = critic(slide);
      if (!issues.length) {
        slide.qa = {
          readability_score: fleschKincaid([slide.title, ...slide.bullets].join('. ')),
          density_score: scoreDensity(slide),
          overflow_risk: scoreDensity(slide) > 0.6 ? 'high' : 'low',
          pass: true,
          fail: []
        };
        break;
      ...
      slide = rewrite(slide);
      slide.revisions_count += 1;
      await log('rewrite', 'retry', { slide_id: id, issues });
    }

    for (const img of slide.images) {
      const stat = await maybeImage(img);
      await log('image', 'ok', { slide_id: id, status: stat });
    }
    slides.push(slide);
  }
  ...
  slides.push({
    slide_id: closeId,
    purpose: 'Close with next gate',
    layout: 'close',
    copy: { kicker: 'Decision', title: 'If aligned, Gate 1 starts here.', subtitle: 'Define scope, authority, and timeline.', footer: 'Sentinel v2' },
    title: 'If aligned, Gate 1 starts here.',
    bullets: [],
    images: [],
    continuity: { from_prev: slides[slides.length - 1]?.title || '', to_next: 'Execution kickoff' },
    constraints: { max_bullets: 6, max_words_per_bullet: 12, target_grade_level: '8-9' },
    provenance: { sources: [] },
    qa: { readability_score: 6, density_score: 0.2, overflow_risk: 'low', pass: true, fail: [] },
    revisions_count: 0
  });
  ...
  for (const s of slides) {
    await renderSlide(deckDir, opts.template_id, s);
    await log('render', 'ok', { slide_id: s.slide_id });
  }
  ...
  const qa = {
    ...
    slides: slides.map((s) => ({ slide_id: s.slide_id, title: s.title, qa: s.qa })),
    summary: { total: slides.length, pass: slides.filter((s) => s.qa?.pass !== false).length, fail: slides.filter((s) => s.qa?.pass === false).length }
  };
  await fs.writeFile(path.join(deckDir, 'qa-report.json'), JSON.stringify(qa, null, 2), 'utf8');
